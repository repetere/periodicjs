    ...
    try {
      ...
      this.crud.ext.list()
        .then(extensions => {
          ...
          if (errors.length) {
            if(Object.keys(this.config.process.argv).filter(prop => [ 'addExtension', 'aex', 'removeExtension', 'rex' ].includes( prop )).length===0) errors.forEach(error => this.logger.error(error));
          }
