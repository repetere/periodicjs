...
function assignSequelizeModels(options) {
  const { modelFiles, modelDirPath, periodic_db_name, db, resolve, reject, } = options;
        ...
        .then(extensions => {
          ...
          extensions.forEach(ext => {
            const requiredExtDependencies = (ext.periodic_dependencies && ext.periodic_dependencies.length)
              ? ext.periodic_dependencies.filter(extDep => extDep.optional !== true).map(extDep => extDep.extname)
              : [];
            
            requiredExtDependencies.forEach(reqExt => {
