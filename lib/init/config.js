...
function configureSequelize() {
  return new Promise((resolve, reject) => {
    try {
      /**
      sqlDB = new Sequelize(dbname,username,password,{
        dialect: 'mysql',
        port: port,
        host: hostname,
        logging: (pas_mysql.env === 'production') ? false : true
      });
       */
      //Basic usage (sql)
      //const sqlConfig = this.config.configuration.options;
      //const sqlDB = new Sequelize(sqlConfig.database, sqlConfig.username, sqlConfig.password);
      //sqlDB.define('configuration', configSchemas.sequelize);
