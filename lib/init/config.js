...
function configureMongoose() {
  return new Promise((resolve, reject) => {
    try {
      ...
      const { server, } = mongooseConfig.mongoose_options || {};
      if (server && (server.sslCA || server.sslCert)) {
        let sslCA;
        let sslCert;
        if (server.sslCA) {
          sslCA = [fs.readFileSync(server.sslCA, 'utf8'), ];
        }
        if (server.sslCert) {
          sslCert = fs.readFileSync(server.sslCert, 'utf8');
        }
        mongooseConfig.server = Object.assign(server, { sslCA, sslCert, });
      }
      ...
      const CoreConfigDataAdapter = this.core.data.create(
        Object.assign({
          adapter: 'mongo',
          db_connection:configurationMongoose,
          model: configurationModel,
        }, schemeDefaults.coreDataOptions.configuration)
      );
      const CoreExtensionDataAdapter = this.core.data.create(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      sqlConfig.connection_options = Object.assign({}, {
        logging: this.config.debug, }, sqlConfig.connection_options);
...
function assignControllers(options) {
  ...
  const engine = require(this.settings.express.views.package);
  // console.log('this.core.controller',this.core.controller)
  const CoreController = new CoreControllerModule(Object.assign({}, this, { engine, }), {
    compatibility: false,
    skip_responder: true,
    skip_db: true,
    skip_protocol: true,
  ...
  // dboptions.controller[ controller_name ].responder.engine = require(this.settings.express.views.package);
  // dboptions.controller[ controller_name ].protocol.engine = require(this.settings.express.views.package);
  // console.log('dboptions.controller[ controller_name ].responder', dboptions.controller[ controller_name ].responder)
  // console.log('dboptions.controller[ controller_name ].responder', ,
  // )
...
function assignMongooseModels(options) {
  ...
  try {
    ...
    modelpaths.forEach(modelFilePath => {
      ...
      if (modelModule.coreDataOptions && modelModule.coreDataOptions.uniqueCompound) {
...
function connectMongooseDB(options) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const { server, } = dboptions.mongoose_options;
      if (server && (server.sslCA || server.sslCert)) {
        let sslCA;
        let sslCert;
        if (server.sslCA) {
        ...
        dboptions.mongoose_options.server = Object.assign(server, { sslCA, sslCert, });
      }
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      Array.from(this.extensions.values()).forEach(extension => {
        ...
        if(this.status) {
          this.status.on('configuration-complete', () => {
            try {
              require(`${extension.name}/resources/async_routers/index`);
            } catch (e) {
              if (this.config.debugAsync) {
                this.logger.warn(`${extension.name} is missing a default async router`, e);
              } else if (e.message !== `Cannot find module '${extension.name}/resources/async_routers/index'`) {
                this.logger.error(e);
              }
            }
          });
      ...
      if (this.settings.container && this.settings.container.name) {
        ...
        if(this.status) {
          this.status.on('configuration-complete', () => {
            try {
              if (this.settings.container.type === 'local') require(`${this.config.app_root}/content/container/${this.settings.container.name}/resources/async_routers/index`);
              else require(`${this.settings.container.name}/resources/async_routers/index`);
            } catch (e) {
              if (this.config.debugAsync) {
                this.logger.warn(`${this.settings.container.name} is missing a default async router`, e);
              } else if (e.message !== `Cannot find module '${this.settings.container.name}/resources/async_routers/index'` && e.message !== `Cannot find module '${this.config.app_root}/content/container/${this.settings.container.name}/resources/async_routers/index'`) {
                this.logger.error(e);
              }
            }
          });
    ...
    try {
      ...
      const StandardCoreController = new CoreControllerModule(this, {
        ...
        responder_configuration: {
          adapter: 'html',
          engine: require(this.settings.express.views.package),
        },
        // skip_responder: true,
        skip_db: true,
        // skip_protocol: true,
