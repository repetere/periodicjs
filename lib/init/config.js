...
function assignControllers(options) {
  ...
  this.controllers.set(`${periodic_db_name}_${controller_name}_${modelName}`, CoreController.db[ modelName ]);
  this.routers.set(`${periodic_db_name}_${controller_name}_${modelName}`, CoreController.protocol.api.implement({
    model_name: modelName,
  }));
  
  if (controller_name === 'default') {
    this.controllers.set(`${periodic_db_name}_${modelName}`, this.controllers.get(`${periodic_db_name}_${controller_name}_${modelName}`));
    this.routers.set(`${periodic_db_name}_${modelName}`, this.routers.get(`${periodic_db_name}_${controller_name}_${modelName}`));
  }
...
function assignLowkieModels(options) {
  const { modelFiles, modelDirPath, periodic_db_name, db, resolve, dboptions, } = options;
  ...
  db.sync({})
    ...
    .then(() => {
      ...
      models.forEach(model => {
        ...
        Object.keys(dboptions.controller).forEach(controller_name => assignControllers.call(this, { dboptions, controller_name, modelName: model.name, CoreConfigDataAdapter, periodic_db_name, }));
