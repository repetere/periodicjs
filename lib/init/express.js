...
function enabledSessionMiddleware(sessionMiddleware, req, res, next) {
  if (skipSessions({ req })) {
    return next();
  } else {
  ...
  return new Promise((resolve, reject) => {
    try {
      if (this.settings.express.sessions.enabled) {
        ...
        const enabledSessionMiddlewareFunc = enabledSessionMiddleware.bind(sessionMiddleware);
        this.app.use(enabledSessionMiddlewareFunc);
...
function useLocalsMiddleware(req, res, next) {
  ...
  this.app.locals.isLoggedIn = (isloggedinoptions) => req.user;
  next();
}
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      this.routers.forEach((value, key)=>{
        console.log("this.utilities.routing.route_prefix(key.replace('_','/'))",this.utilities.routing.route_prefix(key.replace('_','/')));
        if(value.type==='data'){
          this.app.use(this.utilities.routing.route_prefix(key.replace('_','/')),value.router);
        }
      });
...
const catchAllErrorMiddleware = function catchAllErrorMiddleware(err, req, res, next) {
  if (!err) {
    ...
    res.status(500);
    res.render(this.settings.express.views.custom_error_view, {
      user: req.user,
      message: err.message,
      error: err
    });
