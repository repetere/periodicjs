...
function enabledCSRFMiddleware(csrfMiddleware) {
  return (req, res, next) => {
    // console.log('req.headers[this.settings.express.config.skip_csrf_header]', req.headers[this.settings.express.config.skip_csrf_header]);
    if (req.headers[this.settings.express.config.skip_csrf_header]) {
      return next();
    } else {
...
function checkOutdatedExtensionVersions() {
  const extensionNamesToFetch = Array.from(this.extensions.keys());
  const extensions = Array.from(this.extensions.values());
  const registryExtensionFetchs = extensionNamesToFetch.filter(ext => ext.indexOf('@') < 0).map(extname => {
    return new Promise((resolve, reject) => {
  ...
  Promise.all(registryExtensionFetchs)
