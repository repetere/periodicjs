...
try {
  ...
  console.error('Native bcrypt support disabled', e);
  // console.error(e);
}
...
function comparePassword(options) {
  ...
  return new Promise((resolve, reject) => {
    if (!userPassword) return reject(new Error(`Invalid Password (${userPassword})`));
    try {
      bcrypt.compare(userPassword, candidatePassword, (err, isMatch) => {
        if (err) {
          return reject(err);
        } else {
