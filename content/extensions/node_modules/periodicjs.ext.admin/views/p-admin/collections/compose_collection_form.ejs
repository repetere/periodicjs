			...
			<div class="_pea-document" id="collection-items">
				<div id="collection-item-documents" class="_pea-row _pea-container-forminput">
					...
					<%if(collection && collection.items){ %>
						<% collection.items.forEach(function(docresult){%>
							<tr id="tr-docid-<%= docresult.item._id%>">
								<td class="_pea-col-span9">
									<input style="display:none;"  name="items" type="checkbox" value='{"order":<%=docresult.order%>,"item":"<%=docresult.item._id%>"}' checked="checked"></input>
									<%= docresult.item.title %>
									<div><small>
										<%
										if(docresult.item.tags){
											%>
											<strong>tags:</strong> <%
										...
										if(docresult.item.contenttypes){
											%>
