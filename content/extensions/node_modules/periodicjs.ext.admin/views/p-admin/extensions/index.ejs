<% include ../../shared/header %>
<% include ../../shared/nav %>
	<div class="_pea-container">
		<div class="_pea_row">
			<div class="_pea-col-span12">
				<h1>Installed Extensions <small><a href="#add-extensions" class="">(Add Extensions)</a></small></h1>
			</div>
		</div>
		<div class="_pea-row">
			<div class="_pea-col-span12">
				<div class="_pea-document">
					<table id="installed-ext-table" class="_pea-table _pea-no-bottomborder">
						<thead>
							<tr class="_pea-text-left">
								<th>Name</th>
								<th>Version</th>
								<th>Enabled</th>
								<th>Dependencies</th>
							</tr>
						</thead>
						<tbody id="installed-ext-tablebody">
							<% if(extensions.length>0){
									extensions.forEach(function(extension){ %>
										<tr>
											<td> 
												<a href="/p-admin/extensions/<%= extension.name %>/"> <%= extension.name %>
													<%if(extension.author && extension.author.name){%>
														, by <%= extension.author.name %>
													<%}%>
												</a>
												<div>
													<small>
														
														<%if(extension.periodicConfig 
															&& extension.periodicConfig.periodicDependencies
															&& extension.periodicConfig.periodicDependencies.length>0){
															extension.periodicConfig.periodicDependencies.forEach(function(extensiondependency){%>
																<%= extensiondependency.extname %> @ <%= extensiondependency.version %>,
															<%});
														}%>
													</small>
												</div>
											</td>
											<td>
												<%= extension.version %>
											</td>
											<td>
												<%if(extension.enabled){%>
													<a href="/p-admin/extension/<%= extension.name %>/disable">disable</a>
												<%}
												else{%>
													<a href="/p-admin/extension/<%= extension.name %>/enable">enable</a>
												<%}%>
											</td>
											<td>
											delete
											</td>
										</tr>
										<%
									});
								}
								else{ %>
									<tr><td colspan="5" class="_pea-text-center">No Extensions!</td></tr>
								<%} %>
						</tbody>
						<!-- <tfoot>
							<tr class="_pea-text-left">
								<th>Title</th>
								<th>Author</th>
								<th>Categories</th>
								<th>Tags</th>
								<th>Type</th>
								<th>Date</th>
							</tr>
						</tfoot> -->
					</table>
				</div>
			</div>
		</div>
		<div class="_pea-row">
			<div class="_pea-col-span6">
				<h1><a name="add-extensions"></a>Add Extensions</h1>
			</div>
		</div>
		<div class="_pea-row">
			<div class="_pea-col-span12">
				<div class="_pea-document">
					<div class="_pea-row">
						<div class="_pea-col-span9">
							<div class="_pea-row _pea-container-forminput _pea-text-center _pea-form">
								<input type="text" id="search-ext_input" class="_pea-col-span10" placeholder="Search" name="search-ext_input"/>
								<input type="submit" name="search-ext_button" id="search-ext_button" value="Search" class="_pea-button _pea-col-span2 _pea-color-default"/>
							</div>
						</div>
						<div class="_pea-col-span1">
							<div class="_pea-row _pea-container-forminput _pea-text-center">
							or								
							</div>
						</div>
						<div class="_pea-col-span2">
							<div class="_pea-row _pea-container-forminput _pea-text-center _pea-form">
								<input type="submit" name="upload-ext_button" value="Upload" class="_pea-button _pea-col-span12 _pea-color-default"/>
							</div>
						</div>
					</div>
					<table id="ext-search-results" class="_pea-hidden _pea-table _pea-no-bottomborder">
						<thead>
							<tr class="_pea-text-left">
								<th>Name</th>
								<th>Description</th>
								<th>Install</th>
							</tr>
						</thead>
						<tbody id="ext-search-results-tbody">
							
						</tbody>
						<!-- <tfoot>
							<tr class="_pea-text-left">
								<th>Title</th>
								<th>Author</th>
								<th>Categories</th>
								<th>Tags</th>
								<th>Type</th>
								<th>Date</th>
							</tr>
						</tfoot> -->
					</table>
				</div>
			</div>
		</div>
		<div id="ext-console" class="_pea-hidden">
			<div class="_pea_row">
				<div class="_pea-col-span12">
					<h1>Console Output <small><a id="hide-ext-console" class="">(hide)</a></small></h1>
				</div>
			</div>
			<div class="_pea_row">
				<div class="_pea-col-span12" id="ext-console-output">
				</div>
				<hr/>
			</div>
		</div>
		<div class="_pea_row">
			<div class="_pea-col-span12">
				<p>to enable extensions:
					<ul>
						<li>ajax button</li>
						<li>if good, update extensions.json</li>
					</ul>
				</p>
				<p>to view extensions:
					<ul>
						<li>read both packson.json and periodic.ext.json, render two code mirror</li>
					</ul>
				</p>
				<p>to update admin interface:
					<ul>
						<li>put menu under extension name{"periodicjs.ext.admin":{"menu":{"extensions":{"link":"/myextensionviewlink"}}}} in conf</li>
						<li>add "settinglink" in conf</li>
					</ul>
				</p>
			</div>
		</div>
	</div>
	<div style="display:none">
		<div id="view-ext-info-modal">
			<h1 class="title" style="word-wrap: break-word;">Extension name</h1>
			<p class="desc">description</p>
			<p>
				<ul class="versions">
					<li>install <a class="install-ext-version">latest<a></li>
					<li>install 0.0.1</li>
				</ul>
			</p>
		</div>
	</div>
<% include ../../shared/footer %>
