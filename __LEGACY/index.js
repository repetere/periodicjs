...
Promisie.promisify(init.loadCustomStartupConfigurations)(periodic_configure_obj)
    ...
    .then((intialized_return_object) => {
        let application_status_string = `Application Running [ Environment: ${intialized_return_object.periodicSettings.application.environment}`;
        if (intialized_return_object.periodicSettings.application.port) {
            application_status_string += ` | HTTP Port:${intialized_return_object.periodicSettings.application.port}`;
        }
        if (intialized_return_object.periodicSettings.application.https_port) {
            application_status_string += ` | HTTPS Port:${intialized_return_object.periodicSettings.application.https_port}`;
        }
        application_status_string += ` ]`;
        console.log(application_status_string);
    })
    .catch((e) => {
